      ...
      <div className="graph-mgr-section">
        ...
        {knowledgeStatus && (
          <div className="graph-mgr-card-meta" style={{ marginTop: 8 }}>
            {Object.entries(knowledgeStatus).map(([k, v]) => (
              <span key={k} style={{ marginRight: 16 }}>{k}: {typeof v === "object" ? JSON.stringify(v) : String(v)}</span>
            ))}
          </div>
        )}
      </div>
      ...
      {detail && (
        <div className="graph-mgr-section">
          ...
          <div className="graph-mgr-nodes">
            <h4>知识点 ({detail.nodes.length})</h4>
            <div className="graph-mgr-node-list">
              {detail.nodes.map((n) => (
                <div key={n.id} className="graph-mgr-node">
